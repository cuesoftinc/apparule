...
| A7c | Self-host **[Directive 2026-07-18]** | data-ownership pitch (your measurements on your own metal — nothing leaves your server) · tabbed snippet **Docker Compose \| Helm** (CodeSnippetBlock `tabs=docker/helm`): mirrored two-line commands — `git clone https://github.com/cuesoftinc/apparule` then `cd apparule && docker compose up --build -d` (Makefile `up` target) vs `cd apparule && helm install apparule deploy/helm` — only the final command differs **[Ratified 2026-07-20]** · "what ships" list (`api-common` · `api-measure` · `web`, per docker-compose.yml) · link → docs quickstart (GitBook) | copy button ✓ morph (shared with A7); `self_host_click` fires here too |
