...
// ---------------------------------------------------------------------------
// Vault — kiki's measurement sessions (A-10 women template ONLY: the tailor
// vocabulary is the product vocabulary for every method; scans map the full
// template, mock-ahead of the pipeline recalibration). Two scans + one tape
// session give every field ≥2 history points so sparklines render.
// ---------------------------------------------------------------------------
...
export const seedSessions: MeasurementSession[] = [
  {
    ...
    measurements: [
      ...
      },
      {
        ...
        value_cm: 78.5,
        source: "pipeline",
        // Low confidence (<0.7): keeps the MeasurementCard low-confidence
        // chip reachable from boot (B4 canvas exemplar).
        confidence: 0.68,
      },
      {
        id: "m-6",
        session_id: "sess-recent-scan",
        name: "shoulder_to_bust_point",
        value_cm: 26.0,
        source: "pipeline",
        confidence: 0.86,
      },
        ...
        input.createdDaysAgo <= 12
          ? {
              ...
              measurements: [
                ...
                { name: "bust", value_cm: 96.5 },
                { name: "waist", value_cm: 78.5 },
                { name: "gown_length", value_cm: 123.5 },
              ],
            }
          : input.createdDaysAgo <= 58
            ? {
                ...
                measurements: [
                  { name: "shoulder", value_cm: 42.2 },
                  { name: "bust", value_cm: 95.8 },
                  { name: "waist", value_cm: 77.9 },
                  { name: "trouser_length", value_cm: 99.0 },
                ],
              }
            : {
                method: "mediapipe_2d",
                ...
                measurements: [
                  { name: "shoulder", value_cm: 41.8 },
                  { name: "bust", value_cm: 95.2 },
                  { name: "waist", value_cm: 77.4 },
                ],
              },
  ...
  },
  {
    ...
    snapshot: {
      ...
      values: {
        ...
        measurements: [
          { name: "shoulder", value_cm: 28.0 },
          { name: "bust", value_cm: 60.5 },
          { name: "waist", value_cm: 55.0 },
          { name: "top_length", value_cm: 45.0 },
        ],
      },
