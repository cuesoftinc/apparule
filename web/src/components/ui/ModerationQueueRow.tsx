...
// dismiss · state open / actioned (audit: action + actioned_by +
// actioned_at). B7a frame anatomy (audit #19): authored title ("Reported
// post by @amara.designs"), "@reporter" meta, "Actioned · hide_comment by
// @mod · Jul 15, 09:12" audit line.
...
export function ModerationQueueRow({
  ...
  const author = report.subject_preview.author_username;
  // The audit line records the effective action — hide on a comment reads
  // "hide_comment" (B7a frame).
  const actionTaken =
  ...
  return (
    ...
    >
      <div className="flex items-start gap-3">
        ...
        {/* B7a frame: authored headline ("Reported post by @amara.designs"),
            comment excerpt + "@reporter" meta, warn-tinted reason pill. */}
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-0.5 line-clamp-2 text-caption text-text-2">
            {report.subject_kind === "comment"
              ? `“${report.subject_preview.text}” · `
              : ""}
            Reported by @{report.reporter.username} ·{" "}
            {formatAgoPhrase(report.created_at)}
          </p>
        </div>
      ...
      {open ? (
        ...
        <p data-testid="audit-line" className="text-micro text-text-2">
          Actioned{actionTaken ? ` · ${actionTaken}` : ""} by @
          {report.actioned_by ?? "moderator"}
          {report.actioned_at
