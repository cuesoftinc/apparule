...
@ProviderFor(ExploreFollowController)
final exploreFollowControllerProvider = ExploreFollowControllerProvider._();
...
/// is read-only orchestration (no watched state), so autoDispose would
/// unmount its Ref across the repository await.
final class ExploreFollowControllerProvider
    extends $NotifierProvider<ExploreFollowController, void> {
  /// MI-7 follow morph from the C3 designers section — mutates the graph,
  /// then invalidates every explore derivation so `viewer_follows` (and
  /// the C2 feed on its next build) re-derive. keepAlive: the controller
  /// is read-only orchestration (no watched state), so autoDispose would
  /// unmount its Ref across the repository await.
  ExploreFollowControllerProvider._()
    : super(
        from: null,
        argument: null,
        retry: null,
        name: r'exploreFollowControllerProvider',
        isAutoDispose: false,
        dependencies: null,
        $allTransitiveDependencies: null,
      );
  ...
  @override
  String debugGetCreateSourceHash() => _$exploreFollowControllerHash();

  @$internal
  @override
  ExploreFollowController create() => ExploreFollowController();
...
String _$exploreFollowControllerHash() =>
    r'7f7ea6e7684b980e69b315486d30914aebbecf82';
...
abstract class _$ExploreFollowController extends $Notifier<void> {
  void build();
  @$mustCallSuper
  @override
